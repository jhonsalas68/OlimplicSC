...
@if($planificacionesPropias->isEmpty() && $planificacionesOtras->isEmpty())
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-12 text-center">
        ...
        <a href="{{ route('trainings.create') }}" class="mt-3 inline-block text-sm text-blue-600 hover:text-blue-800 font-bold transition-colors">
...
@else
    ...
    @if($planificacionesPropias->isNotEmpty())
        <h2 class="text-sm font-black text-slate-500 uppercase tracking-wider mb-4 px-2">Tu Categoría ({{ $category->nombre ?? '' }})</h2>
    ...
    @if($planificacionesOtras->isNotEmpty())
        <div class="flex items-center gap-3 mb-4 px-2 mt-4">
            <div class="h-px bg-slate-200 flex-1"></div>
            <h2 class="text-xs font-black text-slate-400 uppercase tracking-widest">Otras Categorías</h2>
            <div class="h-px bg-slate-200 flex-1"></div>
        </div>
        <div class="space-y-3">
            @foreach($planificacionesOtras as $plan)
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center gap-4 opacity-80 hover:opacity-100 transition-opacity">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs font-medium text-slate-400 mt-0.5">
                            Entrenador: {{ $plan->coach->name ?? 'Admin' }} &middot; Fecha: {{ $plan->fecha ? $plan->fecha->format('d/m/Y') : '' }}
                        </p>
                    </div>
