...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
    <div class="flex items-center space-x-4 w-full sm:w-auto">
        <form action="{{ route('trainings.index') }}" method="GET" class="w-full sm:w-auto">
            <select name="category_id" onchange="this.form.submit()" class="block w-full sm:w-64 px-4 py-2.5 border border-slate-200 rounded-xl bg-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm transition-all">
    ...
    <x-admin.button onclick="window.location.href='{{ route('trainings.create') }}'" class="w-full sm:w-auto justify-center">
        ...
        <tr class="hover:bg-slate-50/50 transition-colors">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex items-center justify-end space-x-3">
                    @if(auth()->user()->hasRole('Admin') || auth()->user()->hasRole('SuperAdmin') || auth()->id() == $training->coach_id)
                    <a href="{{ route('trainings.edit', $training) }}" 
                       class="p-2 text-blue-600 hover:text-blue-900 transition-colors" title="Editar">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                    </a>
                    <form action="{{ route('trainings.destroy', $training) }}" method="POST" 
                          onsubmit="return confirm('¿Eliminar esta planificación?')" class="inline">
                        @csrf @method('DELETE')
                        <button type="submit" class="p-2 text-red-600 hover:text-red-900 transition-colors cursor-pointer" title="Eliminar">
                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
